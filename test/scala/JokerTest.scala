...
class JokerTest extends FunSuite {
  ...
  override def beforeEach(context: BeforeEach): Unit = {
    ...
    AceHeart = new Card(new Ace, new Heart)
  ...
  test("different jokers type test"):
    assert(DeviousJ != EvenJ)
    assert(DeviousJ != ScaryF)
    assert(DeviousJ != GreedyJ)
    assert(EvenJ != ScaryF)
    assert(EvenJ != GreedyJ)
    assert(GreedyJ != ScaryF)
  ...
  test("Playing cards with empty Jokers list"):
    TwoDiamond.applyScore(score, List())
    assertEquals(score.chips,2)
    assertEquals(score.multiplier,0)
